...
`@kjfsm/musescore-plugin-sdk-helpers` や `@kjfsm/musescore-plugin-sdk-types` などの外部 SDK ライブラリに必要な機能（型定義・述語・ユーティリティ関数など）が存在しない場合は、**このプラグイン側で回避実装をしてはいけない**。
作業を止め、以下を明示してユーザーに報告すること。

1. **不足している機能の具体名**（例: `isHairpin()` 述語、`Hairpin` インターフェース 等）
2. **どのパッケージに追加が必要か**（例: `@kjfsm/musescore-plugin-sdk-helpers`）
3. **なぜ不足しているとみなしたか**（型定義の確認結果など）

### `.claude/` を編集するときのルール

`.claude/` 以下のファイル（`settings.json`、`skills/` など）を新規作成・変更する場合は、**必ず次の手順**で行う。

1. `.claude-next/` に作業用ファイルをすべて作成・編集する
2. 内容を確認してから、最後に一括で `.claude/` へコピーする

```bash
# 作業例
mkdir -p .claude-next/skills
# ... ファイルを .claude-next/ 以下で編集 ...
cp -r .claude-next/. .claude/
rm -rf .claude-next/
```
