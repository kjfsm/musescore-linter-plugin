...
| 3    | **拍子**（配線済み）           | `getMeasureTimeSig`（既存）を利用                                                                            | `meta.measures[]`、snapshot / MusicXML 双方に配線                    | F3 拍境界またぎ（実装済み）、F1 拍頭の休符、F2   |
...
`@kjfsm/musescore-plugin-sdk-helpers@2.2.0` 公開後、プラグインで以下を実施済み:

1. `packages/core/package.json` の `@kjfsm/musescore-plugin-sdk-helpers` を `^2.2.0` に更新して `pnpm install`。
2. `packages/core/src/snapshot.ts` の chord 処理に音高・タイの抽出を配線（下記）。
   これにより `tie-pitch-mismatch` / `courtesy-accidental` / `repeat-barline-match` が実楽譜でも発火する。
