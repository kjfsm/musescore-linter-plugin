...
class PartWalker {
	...
	private staffIdxOf(staffNumber: number): number {
		const clamped = Math.min(
			Math.max(staffNumber, 1),
			Math.max(this.ctx.staffCount, 1),
		);
	...
	private readNoteInfo(node: XNode, staffNumber: number): NoteInfo | null {
		...
		return {
			...
			line: toStaffLine(
				this.clefs.get(staffNumber) ?? DEFAULT_CLEF,
				step,
				octave,
			),
		...
		for (const notations of childrenNamed(node, "notations")) {
			for (const tied of childrenNamed(notations, "tied")) {
				...
				if (type === "start") {
					...
					const openKey = exact
						? key
						: this.findLooseTieKey(staffIdx, voice, pitch);
	...
	private findLooseTieKey(
		...
		for (const [key, span] of this.openTies) {
			...
			const distance =
				pitch === null || span.startPitch === null
					? 0
					: Math.abs(span.startPitch - pitch);
	...
	private handleDirection(node: XNode): void {
		...
		const text =
			words.join(" ").trim() || (metronome ? metronomeText(metronome) : "");
		if (text === "") return;
...
function assemble(draft: ScoreDraft): LintIR {
	...
	const absolute = (p: Placed): number => {
		...
		if (p.atMeasureEnd)
			return start + (draft.measureLengths[p.measureIdx] ?? 0);
