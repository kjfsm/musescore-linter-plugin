import type { Checker, LintIR, Issue } from "@musescore-linter/core";
  ...
  run(ir: LintIR): Issue[] {
    ...
    for (const staff of ir.meta.parts) {
      ...
      for (const id of tickIds) {
        ...
        if (isDynamicMark(tev, ir)) { hasDynamics = true; break; }
      }
      ...
      if (!hasDynamics) {
        ...
        for (const id of globalIds) {
          ...
          if (isDynamicMark(gev, ir)) { hasDynamics = true; break; }
        }
      }
