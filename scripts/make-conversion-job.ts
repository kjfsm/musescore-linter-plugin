...
export function findScores(dir: string): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (SCORE_EXTENSIONS.includes(path.extname(entry.name).toLowerCase())) {
      found.push(full);
    }
  }
  return found.sort();
...
export function buildJob(inputDir: string, outputDir: string): { in: string; out: string }[] {
...
function main(): void {
  const [inputDir, outputDir, jobPath = "conversion-job.json"] = process.argv.slice(2);
  ...
  console.error(`${job.length} 件の楽譜を ${outputDir} へ変換するジョブを書きました`);
