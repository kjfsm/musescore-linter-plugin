...
describe("loadEnabledRules / saveEnabledRules", () => {
	...
	it("既定と同じ値は保存しない（checker が増えても既定に追従させるため）", () => {
		...
		const raw = JSON.parse(
			storage.getItem("musescore-linter:rule-overrides") ?? "{}",
	...
	it("壊れた値は捨てて既定に戻す", () => {
		expect(
			loadEnabledRules(fakeStorage({ "musescore-linter:rule-overrides": "{" })),
		).toEqual({});
		expect(
			loadEnabledRules(
				fakeStorage({ "musescore-linter:rule-overrides": "[1]" }),
			),
		).toEqual({});
	});
});
