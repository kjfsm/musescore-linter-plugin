...
| 休符上のスパナー端点                           | warning  | ヘアピン(cresc./dim.)やスラーの端点が休符上にある（同 tick に音符があれば許容、ヘアピンの終了端点は info） |
...
push または PR をオープンすると GitHub Actions が lint / フォーマット（oxlint + oxfmt）・unused 検出（knip）・typecheck・テスト・ビルドを自動実行します。
