...
var __bundle__ = (() => {
  ...
  function buildEnumRegistry(E) {
    ...
    function resolveElementKind(rawType) {
      ...
      if (enums.CHORD !== void 0 && rawType === enums.CHORD)
        return CANONICAL.elementKinds.CHORD;
      if (enums.REST !== void 0 && rawType === enums.REST)
        return CANONICAL.elementKinds.REST;
      if (enums.BAR_LINE !== void 0 && rawType === enums.BAR_LINE)
        return CANONICAL.elementKinds.BAR_LINE;
      if (enums.TEMPO_TEXT !== void 0 && rawType === enums.TEMPO_TEXT)
        return CANONICAL.elementKinds.TEMPO_TEXT;
      if (enums.STAFF_TEXT !== void 0 && rawType === enums.STAFF_TEXT)
        return CANONICAL.elementKinds.STAFF_TEXT;
      if (enums.SYSTEM_TEXT !== void 0 && rawType === enums.SYSTEM_TEXT)
        return CANONICAL.elementKinds.SYSTEM_TEXT;
      if (enums.EXPRESSION !== void 0 && rawType === enums.EXPRESSION)
        return CANONICAL.elementKinds.EXPRESSION;
      if (enums.REHEARSAL_MARK !== void 0 && rawType === enums.REHEARSAL_MARK)
        return CANONICAL.elementKinds.REHEARSAL_MARK;
      if (enums.DYNAMIC !== void 0 && rawType === enums.DYNAMIC)
  ...
  function buildSnapshot(score, E) {
    ...
    while (m) {
      try {
        ...
        log2.warn(`measure ${measureNum} \u306E\u89E3\u6790\u4E2D\u306B\u30A8\u30E9\u30FC: ${e}`);
      }
      measureNum++;
      m = m.nextMeasure;
    }
  ...
  var openingTempoChecker = {
    ...
    run(ir) {
      ...
      const firstMusicEvent = [...chordIds, ...restIds].map((id) => ir.events[id]).reduce((best, ev) => {
        if (!best) return ev;
    ...
    run(ir) {
      ...
      for (const staff of parts) {
        ...
        const restTicks = new Set(
          ((_d = byStaff[canonical.elementKinds.REST]) != null ? _d : []).map(
  ...
  var tempoBarlineChecker = {
    ...
    run(ir) {
      ...
      tempoEvents.sort(
        (a, b) => a.tick !== b.tick ? a.tick - b.tick : a.measure - b.measure
      );
      barlineEvents.sort(
        (a, b) => a.tick !== b.tick ? a.tick - b.tick : a.measure - b.measure
      );
